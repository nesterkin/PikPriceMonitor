...
object Main {
    ...
    private val prettyFlats = responseDir + "PrettyAllFlats.json"
    private val gson = GsonBuilder().setPrettyPrinting().create()

    @JvmStatic
    fun main(args: Array<String>) {
        val flats = if (File(prettyFlats).exists()) {
            println("Cache mode")
            val reader = BufferedReader(FileReader(prettyFlats))
            val rawJson = reader.readText()
            gson.fromJson<FlatWrapper>(rawJson, FlatWrapper::class.java).flats
        } else {
            println("Internet mode")
            makeHttp()
            ...
            do {
                ...
                val prettyWriter = BufferedWriter(FileWriter(responseDir + "PrettyPage$i.json"))
...
//            .filter { it.rooms == "2" }
//            .filter { it.bulk.settlement_year == 2020 }
            .forEach {
                outputFlats.add(
                    PrintableFlat(
